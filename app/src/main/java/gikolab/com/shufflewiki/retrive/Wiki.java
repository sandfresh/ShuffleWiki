...
public class Wiki
{
  ...
  List<IupdateListener> mListenerList;
  SaveFile mSave;
  public Wiki()
  {
    mBookTitleList = new ArrayList<>();
    mListenerList = new ArrayList<IupdateListener>();
  }
  ...
  {
    String url =
        "query&list=random&format=json&rnnamespace=0&rnlimit=10&rawcontinue=&generator=random&grnnamespace=0";
  ...
  {
    String url =
        "query&prop=imageinfo&format=json&iiprop=url&rawcontinue=&generator=images&titles=" + value;
  ...
  private class getImageThread extends Thread
  {
	...
	public void notify(int action, String value)
	{
		switch (action)
		{
			case 0:
				for (IupdateListener listener : mListenerList)
				{
					listener.onAddData(value);
				}
				break;
			case 1:
				for (IupdateListener listener : mListenerList)
